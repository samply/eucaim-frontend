...
import { markSiteClaimed, setSiteResult, type LensResult } from '@samply/lens';
import { resultsStore, transformResponse } from './backend.service';

export function mockQuerySpot() {
	const mockResponse: Provider[] = getMockResponse();
	mockResponse.forEach((provider) => {
		const parsedResponse: LensResult = transformResponse(provider);
		markSiteClaimed(provider.provider);
		setSiteResult('site' + provider.provider, parsedResponse);
	});
	resultsStore.set(mockResponse);
}

function getMockResponse(): Provider[] {
	return [
		{
			provider: 'ProCancerI',
			provider_icon:
